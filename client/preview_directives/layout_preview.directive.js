(function() {
	...
	function previewDirective() {
		return {
			templateUrl:'restaur_content/directive_view2.html',
			restrict: 'EA',
			controller: PreviewController			
		}
	}

	function PreviewController($scope, RestaurFactory, $location, $uibModal, $log, $rootScope) {
		var tables,
		icons,
		...
		$rootScope.$on('changeTable', function(event, data){
			console.log(data)
			var theclass = data.tabId
			console.log(theclass)
		})

		$rootScope.$on('addSubmittedOrder', function(event, data){
			console.log('what is my table number')
			console.log(data)
			$('.'+data.tabId).addClass("ordered")
		})
		...
		function addTable(e) {
			...
			e = "<div class=res id=" + tempId + "><ul class='icon_heading'><span>Table " + table_number + "</span></ul><img class='icon_img ready'></img></div>";
